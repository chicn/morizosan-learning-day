...
int main(int argc, char *argv[]){
    ...
    cv::Mat m2(3, 3, CV_MAKE_TYPE(CV_64F, 10));

    CV_Assert(m1.type() == m2.type());
    std::cout << "mat1/mat2" << std::endl;
    std::cout << "dims: " << m1.dims << ", depth(byte/channel): " << m1.elemSize() << ", channels: " << m1.channels() <<std::endl;

    // 64F, channels = 1, 2x2x3x4 の4次元配列
    ...
    std::cout << "  dims: " << m3.dims << ", depth(byte/channel): " << m3.elemSize() << ", Channels: "  <<m3.channels() << std::endl;
}